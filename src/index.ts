...
const checkPlayers = async () => {
    members.map(async (player: { playerId: string; }, index: any) => {
    ...
    await axios.get(`https://gameinfo.albiononline.com/api/gameinfo/guilds/${process.env.GUILD_ID}/members`)
        .then((response: { data: any[]; }) => {
            response.data.map((element: { Id: any; }, index: any) => {
                if (members.findIndex((member: { playerId: any; }) => member.playerId == element.Id) == -1) {
...
const drawEvent = async ({ Killer, Victim, TotalVictimKillFame, Participants, TimeStamp, EventId, color }: DrawEvent) => {
    ...
    let victimEq = Victim.Inventory.filter((value: any) => {
        return value != null
    })
    ...
    if (victimEq.length > 0) {
        ...
        for (let element of victimEq) {
            ...
            if (element != 0) {
                await loadImage('https://render.albiononline.com/v1/item/' + element.Type + '.png?count=' + element.Count + '&quality=' + element.Quality).then((image: any) => {
